...
- feat: `JSONField` supports the `contains` and `contained_by` lookups, with the semantics of the postgres operators django names them after. ClickHouse has no containment operator, but the value is known while the query is compiled, so walking it unrolls the containment into the query: an object descends through the sub-column accessors, and an array is compared with `arrayExists()`/`arrayAll()` over the JSON text of the path. The one difference to postgres is that an object only matches inside an array when it is wrapped in a list, `filter(json__a__contains=[{'b': 1}])`, reported as the `json_key_contains_list_matching_requires_list` feature, as on oracle. A value which is an expression rather than a literal raises `NotSupportedError`.
- feat: support django 6.1, which the test suite now runs against too. It needs python 3.12 or later.
- feat: django's `JSONObject()` and `JSONArray()` (django 5.2 and later) build a JSON value out of their arguments, which clickhouse has no function of its own for: each argument is rendered as its JSON text and the result is cast to JSON, so a key path or a whole JSON column nests into one. An array cannot be the root of a JSON value, so `JSONArray()` is cast inside a wrapper object and read back out of it. A key whose value is `null` is dropped, the way it is in a value saved into the field, while an array keeps its nulls.
- feat: a key path is compared with a JSON value, `filter(json__c=JSONObject(d=Value('e')))` and `filter(json__c=Value({'d': 'e'}, JSONField()))`, by reading the value as JSON text the way the path itself is read. Such a comparison used to match nothing: clickhouse compares a JSON value with a string as never equal.
- feat: a negative array index reads from the end, `values('json__a__-1')` is the last element of `a`, as it is on postgres. It used to be read as the name of a key.
- fix: `JSONField` compiles an expression saved into it, `create(json=JSONObject(a=1))`, instead of passing it to the driver as a value. Django 6.1 looks up the placeholder of a field before compiling an expression, so only django 6.0 and below were unaffected.
