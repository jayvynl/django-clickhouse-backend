...
- feat: #166 the `startswith` and `istartswith` lookups now compile to ClickHouse's native prefix functions instead of `LIKE`/`ILIKE`, based on a PR from @AhmedIbrahim226. `istartswith` uses `startsWithCaseInsensitiveUTF8()` on ClickHouse [25.10](https://clickhouse.com/docs/whats-new/changelog/2025#2510) and later, which added it, and `startsWith(lowerUTF8(...), lowerUTF8(...))` on older servers. Both match what `ILIKE` matched, but a literal prefix no longer needs its `%` and `_` escaped.
