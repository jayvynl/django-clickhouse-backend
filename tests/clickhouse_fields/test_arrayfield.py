...
class ArrayFieldTests(TestCase):
    def test_disallow_nullable(self):
        field = models.ArrayField(
            base_field=models.Int8Field(name="field"), null=True, name="field"
        )
        self.assertEqual(
            field.check()[0].msg, "Nullable is not supported by ArrayField."
        )

    def test_check_base_field(self):
        field = models.ArrayField(
            base_field=models.DateTime64Field(precision=10, name="field"), name="field"
        )
        self.assertTrue(
        ...
        field = models.ArrayField(
            base_field=models.ArrayField(
                base_field=models.EnumField(name="field"), name="field"
            ),
            name="field",
        )
        self.assertTrue(
    ...
    def test_value(self):
        ...
        o.save()
        o.refresh_from_db()
        self.assertEqual(o.array, ["1.2.3.4", "2.3.4.5"])
    ...
    def test_filter(self):
        ...
        self.assertEqual(ArrayModel.objects.filter(array=v)[0].array, v)

        self.assertTrue(ArrayModel.objects.filter(array__any="2.3.4.5").exists())
        self.assertFalse(ArrayModel.objects.filter(array__any="4.5.6.7").exists())
        ...
        self.assertTrue(ArrayModel.objects.filter(array=v).exists())
        self.assertFalse(
            ArrayModel.objects.filter(
                array=[
                    "2.3.4.5",
        ...
        self.assertFalse(
            ArrayModel.objects.filter(
                array__overlap=["3.4.5.6", "ed51:aaa0:3c7e:a65a:bb72:82b7:4d87:44dd"]
            ).exists()
        )

        self.assertTrue(ArrayModel.objects.filter(array__len=3).exists())
        self.assertFalse(ArrayModel.objects.filter(array__len=1).exists())

        self.assertTrue(ArrayModel.objects.filter(array__1="2.3.4.5").exists())
        self.assertFalse(ArrayModel.objects.filter(array__0="2.3.4.5").exists())
    ...
    def test_nested(self):
        ...
        self.assertEqual(NestedArrayModel.objects.filter(array=v)[0].array, v)

        self.assertTrue(
            NestedArrayModel.objects.filter(array__any=[[12, 13, 0, 1], [12]]).exists()
        )
        self.assertFalse(NestedArrayModel.objects.filter(array__any=[[12]]).exists())

        self.assertTrue(
            NestedArrayModel.objects.filter(
                array__contains=[[[12, 13, 0, 1], [12]]]
            ).exists()
        )
        self.assertFalse(
            NestedArrayModel.objects.filter(
                array__contains=[[[12, 13, 0, 1], [13]]]
            ).exists()
        )
        ...
        self.assertTrue(NestedArrayModel.objects.filter(array__len=2).exists())
        self.assertFalse(NestedArrayModel.objects.filter(array__len=1).exists())

        self.assertTrue(
            NestedArrayModel.objects.filter(
                array__1=[[12, 13, 0, 1], [12], [13, 14]]
            ).exists()
        )
        self.assertFalse(
            NestedArrayModel.objects.filter(
                array__1=[[12, 13, 0, 1], [13, 14]]
            ).exists()
        )

        self.assertTrue(
            NestedArrayModel.objects.filter(
                array__1_2=[[[12, 13, 0, 1], [12], [13, 14]]]
            ).exists()
        )
        self.assertFalse(
            NestedArrayModel.objects.filter(
                array__1_2=[[[12, 13, 0, 1], [13, 14]]]
            ).exists()
        )

        self.assertTrue(NestedArrayModel.objects.filter(array__1__2=[13, 14]).exists())
        self.assertTrue(NestedArrayModel.objects.filter(array__1__2__0=13).exists())
